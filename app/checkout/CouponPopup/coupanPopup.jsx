...
export default function CouponsPopup({ cartValue, onApply, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchCoupons = async () => {
      try {
        const res = await axiosClient.get("/api/surge-coupon/coupons");
        const data = res.data;
        if (data.success && Array.isArray(data.docs)) {
          const now = new Date();
          const active = data.docs
            .filter((c) => {
            ...
            .map((c) => {
              ...
              if (c.discountType === "percentage") {
                const saved = cartValue * (Number(c.discountAmount) / 100);
                savings = `You save AED ${saved.toFixed(0)}`;
              } else {
                savings = `You save AED ${Number(c.discountAmount).toFixed(0)}`;
              }
              const minText = c.minimumAmount > 0 ? ` on orders above AED ${c.minimumAmount}` : "";
              return {
                id: c.id,
                code: c.code.trim(),
                savings,
                description: c.discountType === "percentage"
                  ? `${c.discountAmount}% off${minText}`
                  : `AED ${c.discountAmount} flat off${minText}`,
      ...
      <div className={styles.drawer} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.inputRow}>
          ...
          <button
            ...
            disabled={!search.trim()} // <-- Disables button if input is empty or just spaces
            onClick={() => {
              const match = coupons.find(c => c.code.toLowerCase() === search.trim().toLowerCase());
        ...
        {loadingCoupons ? (
          <p style={{ textAlign: "center", padding: "20px", color: "#818686" }}>Loading coupons...</p>
        ) : filtered.length === 0 ? (
          <p style={{ textAlign: "center", padding: "20px", color: "#818686" }}>No coupons available</p>
        ) : (
